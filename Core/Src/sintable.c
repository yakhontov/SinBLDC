...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	GPIOB->ODR ^= 0b10; // Debug

	// Определяем какой из регистров изменился и по нему определяем время последнего импульса
	if(prevCCR[0] != currentCCR[0])
		currentTime = currentCCR[0];
	else if(prevCCR[1] != currentCCR[1])
		currentTime = currentCCR[1];
	else if(prevCCR[2] != currentCCR[2])
		currentTime = currentCCR[2];
